...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<"pending" | "rejected" | "members">("pending");
            ...
            <table className="w-full">
              <thead className="bg-light-bg">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-sm font-semibold text-black">{activeTab === 'members' ? 'Role' : 'Company'}</th>
              ...
              <tbody className="divide-y divide-border-light">
                {activeTab === 'pending' && pendingData?.data?.businesses?.map((business: any) => (
                  <tr key={business.id}>
                ...
                {activeTab === 'rejected' && rejectedData?.data?.businesses?.map((business: any) => (
                  <tr key={business.id}>
                    <td className="px-6 py-4 text-black">{business.name}</td>
                    <td className="px-6 py-4 text-black/70 text-sm">{business.email}</td>
                    <td className="px-6 py-4 text-black/70 text-sm">{business.company}</td>
                ...
                {activeTab === 'members' && membersData?.data?.members?.map((member: any) => (
                  <tr key={member.id}>
                    <td className="px-6 py-4 text-black">{member.name}</td>
                    <td className="px-6 py-4 text-black/70 text-sm">{member.email}</td>
                    <td className="px-6 py-4 text-black/70 text-sm">{member.role}</td>
                    <td className="px-6 py-4">
                ...
                {((activeTab === 'pending' && pendingData?.data?.businesses?.length === 0) ||
                  (activeTab === 'rejected' && rejectedData?.data?.businesses?.length === 0) ||
                  (activeTab === 'members' && membersData?.data?.members?.length === 0)) && (
                  <tr>
