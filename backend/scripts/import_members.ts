...
const importMembers = async () => {
  try {
    ...
    for (const data of membersData) {
      const emailRaw = (data["Email"] || data["Email Address"] || "")
        .trim()
        .toLowerCase();
      if (!emailRaw) {
        console.warn(
          `⚠️ Skipping record for ${data["Name"]} - No email provided.`,
        );
        skipped++;
        continue;
      }
      ...
      const phoneStr =
        phoneVal !== undefined && phoneVal !== null
      ...
      const newMember = new CommunityMember({
        ...
        email: emailRaw,
        role: data["Company & Designation"]
          ? String(data["Company & Designation"]).trim()
          : undefined,
        bio: data["Tell us about yourself"]
          ? String(data["Tell us about yourself"]).trim()
          : undefined,
        phone: phoneStr,
        linkedin: data["LinkedIn Profile URL"]
          ? String(data["LinkedIn Profile URL"]).trim()
          : undefined,
        instagram: data["Instagram Profile URL\n"]
          ? String(data["Instagram Profile URL\n"]).trim()
          : undefined,
        photoUrl: data["Professional Photo"]
          ? String(data["Professional Photo"]).trim()
          : undefined,
        status: "active",
      });
